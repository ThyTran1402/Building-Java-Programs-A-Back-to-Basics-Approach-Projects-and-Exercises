...
public class Permutations 
{
    ...
    public static ArrayList<String> permutations(String word)
    {
        ...
        if (word.length() == 0)
        {
            ...
            {
                //Form a shorter word y removing the ith character 
                String shorter = word.substring(0, i) + word.substring(i + 1);
                
                //Generate all permutations of the simpler word
                ArrayList<String> shorterPermutations = permuations(shorter)
                        
                //Add the removed character to the front of
                //each permutation of the simpler word
                for (String s : dhorterPermutations)
                {
                    result.add(word.charAr(i) +s);
                }
